...
import static com.errorgon.sensitivitytest.math.vector.doubles.dense.operation.CreateVector.foreach;
...
//            throw new RuntimeException("could not converge after maximal number of iterations");
        ...
        private Vector mu(Vector eta) {
            return foreach(eta, new UnivariateRealFunction() {
                ...
                public double evaluate(double x) {
                    double mu = link.inverse(x);
                    double eps = 2.2204460492503279e-16;
                    return (Math.abs(mu) < eps)? eps : mu; //handles numerical instability
                }
            });
        }
        ...
        private Vector beta1(Vector eta, Vector mu, Vector weights, Vector dg) {
            ...
            com.errorgon.sensitivitytest.math.stats.regression.linear.LMProblem wols = new com.errorgon.sensitivitytest.math.stats.regression.linear.LMProblem(z, problem.A, false, weights);
